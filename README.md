...
Anti patterns are patterns, many times common patterns, that even if popular, are patterns that should be avoided as they are patterns that go against either best user intentions, best technological intentions, best business intentions, or a combination of all three. There are three big places where anti patterns tend to pop up, those are:
...
* Do not group CSS by media in `<link>` tags; all of the CSS gets downloaded anyways. Instead, reduce the number of aggregates and wrap internal CSS in media queries.
* Use a CDN
* Cache page requests
...
* If a script does not come with a minified version, determine if it can be minified. All scripts should be minified, so if a script being evaluated can not, that should be taken into consideration.
